...
```
Participants sheet (source of truth)
  │
  ├─ Phase 1 reads:  Status, Tier, Group Rank
  │   writes: Tier (fill-promoted players), Group #, Wins Row, Losses Col, K1 (scores sheet name), L1 (tier order)
  │
  ├─ Scores sheet (generated by Phase 1, filled in by players/bot)
  │   contains: win/loss matrix per group block with formula-computed Win%, Rank, Completion
  │
  └─ Phase 2A reads: Scores sheet (Win%, head-to-head matrix), Participants (tier/group assignments)
      writes (always):  Group Rank (objective match finish position)
      writes (commit):  Status, Tier (promotions/demotions/DNFs) + creates _ParticipantsBackup
```
...
| L | *(tier order)* | Written by the extension on Phase 1 — comma-separated tier names in configured order (e.g. `S,A,B,C,D`). Read by Phase 2A to identify which tier is above/below each player. |
...
**Group Rank:** automatically written by Phase 2A (both Preview and Commit) with each player's final rank within their group (1 = top). The extension uses this to seed groups in the next Phase 1 run, interleaving players across groups so each group gets a mix of skill levels. New players start at 0 and are placed last in seeding order.
...
- `K1` — name of the newly created Scores sheet
- `L1` — comma-separated tier order (e.g. `S,A,B,C,D`) used by Phase 2A to identify tier adjacency
...
1. Click **Preview** to see which players promote, demote, or are flagged DNF.
2. Review the list.
3. Click **Commit** to apply tier/status changes to Participants.

**Group Rank is written on both Preview and Commit.** Ranks are an objective result of the score matrix and are written immediately so that a subsequent Phase 1 run (even without committing tier changes) uses up-to-date rank data for group seeding and fill-promotion ordering.

**Before committing**, the extension automatically saves a full copy of Participants to a hidden `_ParticipantsBackup` sheet. Rank writes happen before the backup, so the backup reflects post-rank state.

**DNF logic:** a player is marked DNF if they have played fewer matches than `(N-1) × matchesPerSet` (where N is their group size). DNF players sit out the next rotation with their tier set to the tier below (or same tier if already at the bottom), and automatically return to ACTIVE the rotation after that.
...
**Rollback:** if a backup exists from the last commit, a **Rollback** button appears in the sidebar. Clicking it restores Participants to exactly the state it was in before the last commit (including reversing rank writes). The backup is overwritten on every new commit, so rollback only covers the most recent one.
...
**Fill promotion:** if a tier with `desired_groups` set doesn't have enough players to reach `desired_groups × min_size`, players from the tier immediately below (and further below if still short) are temporarily promoted to fill it. These players may revert next rotation.

Fill promotion candidate priority (within each source tier):
1. Players who have a group and rank from last cycle — sorted by best rank first (rank 1 = highest)
2. Players with no group/rank (new or returning after a gap) — lowest priority

This ensures the most qualified players from the lower tier are promoted rather than defaulting to unranked newcomers.

A second pass also absorbs "stragglers" — if all remaining players across every tier below a given tier total fewer than `min_size`, they cannot form a valid group and are pulled up into the tier above using the same priority ordering.
